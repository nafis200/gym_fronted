    ...
    <div className="max-w-7xl mx-auto space-y-8 p-4 md:p-8 animate-in fade-in duration-700 transition-colors ">
      ...
      <div className="relative overflow-hidden rounded-[2rem] bg-slate-900 dark:bg-slate-950 p-8 md:p-12 shadow-2xl transition-all border border-slate-200 dark:border-slate-800">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-4">
            <Badge
              variant="outline"
              className="bg-blue-500/10 text-blue-400 border-blue-500/20 mb-2"
            >
            ...
            <p className="text-slate-400 text-lg max-w-lg font-medium leading-relaxed">
              Monitor your assets, manage the community, and track your hotel
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((s, i) => (
          <Card
            key={i}
            className="group hover:scale-[1.02] transition-all duration-300 border border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm"
          >
            <CardContent className="p-6">
              ...
              <div className="mt-6">
                ...
                <h2 className="text-4xl font-bold mt-2 text-slate-900 dark:text-white tracking-tight">
                  {s.value}
                </h2>
              </div>
            </CardContent>
        ...
        <div className="p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 dark:border-slate-800">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400">
              Manage and preview your recently uploaded media
            </p>
          </div>
          ...
          {images.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {images.slice(0, 2).map((img) => (
                <div
                  key={img.id}
                  className="group relative aspect-video overflow-hidden rounded-[1.5rem] border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950"
                >
                  <img
                    src={img.url}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 pb-10">
        <Card className="lg:col-span-3 border border-slate-200 dark:border-slate-800 shadow-xl rounded-3xl bg-white dark:bg-slate-900">
          <CardHeader className="p-8 border-b border-slate-100 dark:border-slate-800">
            ...
            <CardDescription>
              Network with {users.length} active platform members
            </CardDescription>
          </CardHeader>
          <CardContent className="p-0">
            <div className="max-h-[500px] overflow-y-auto custom-scrollbar">
              {users.slice(0, 6).map((u) => (
                <div
                  key={u.id}
                  className="flex items-center justify-between p-6 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-all group cursor-pointer"
                >
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <p className="font-bold text-slate-900 dark:text-white group-hover:text-blue-500 transition-colors">
                        {u.name}
                      </p>
          ...
          <div className="space-y-6 relative z-10">
            ...
            <div>
              ...
              <p className="text-blue-100/80 dark:text-white/80 mt-4 text-lg leading-relaxed">
                Get access to powerful analytics, priority support, and
                unlimited storage for your visual assets.
              </p>
            </div>
          </div>
